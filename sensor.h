...
int readSensor(){
     ...
     if(flag == false && d < 10 && d > 4){   // si la medida es menor a 20 y mayor a 5 entonces se dice que paso 1 pelota
        anterior = d;
        flag = true;
        pelotas ++;
        Serial.println(d);     // imprimimos la distancia para corroborar
     }
     else if(flag ==true && anterior > 10 && d < distance && d > 4){  //si la medida es mayor a 20 o menor a 5 , se dice que nada ah pasado 
        flag = false;
     }
     else if(flag ==true && (d > distance || d < 4)){ //si la medida es menor a 20 y mayor a 5 pero la anterior media tambien cumple
